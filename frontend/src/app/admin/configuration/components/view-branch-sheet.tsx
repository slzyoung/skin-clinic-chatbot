...
import { RiEyeLine, RiEdit2Line, RiDeleteBinLine } from "@remixicon/react"
import { DOCTOR_DATA } from "@/dummy/users"
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent className="sm:max-w-100 w-full p-0 flex flex-col gap-0 border-l border-black-50 bg-white">
          <SheetHeader className="p-4 border-b border-black-50">
            <SheetTitle className="text-base font-medium text-black-500 text-left">Branch Information</SheetTitle>
          </SheetHeader>
          ...
          <div className="flex-1 overflow-y-auto pb-6">
            ...
            <Tabs defaultValue="information" className="w-full">
              <div className="px-6 pt-4 border-b border-black-50">
                <TabsList variant="line" className="w-full justify-start h-auto p-0 bg-transparent gap-6">
                  <TabsTrigger 
                    value="information"
              ...
              <TabsContent value="information" className="p-6 m-0 flex flex-col gap-6">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <span className="text-sm font-medium text-black-500 leading-relaxed">{branch.address}</span>
                </div>

                <div className="flex items-center gap-4">
                  <div className="flex flex-col gap-1 flex-1">
                    <span className="text-sm text-black-300">Latitude</span>
                    <span className="text-sm font-medium text-black-500">7&deg;45&apos;32.2&quot;S</span>
                  </div>
                  <div className="flex flex-col gap-1 flex-1">
                    <span className="text-sm text-black-300">Longitude</span>
                    <span className="text-sm font-medium text-black-500">110&deg;23&apos;57.3&quot;E</span>
                  </div>
                </div>

                <div className="flex flex-col gap-1">
                  <span className="text-sm text-black-300">Tokens</span>
                  <span className="text-sm font-medium text-black-500">{branch.tokensMonth} /month</span>
                </div>
                ...
                <div className="bg-blue-50 rounded-lg p-4 flex flex-col gap-3">
                  ...
                  <div className="flex flex-col gap-2">
                    <div className="flex justify-between items-center text-sm font-medium">
                      <span className="text-black-500">{branch.remaining}/{branch.tokensMonth} tokens left</span>
                    </div>
                    <div className="h-2 w-full bg-black-50 rounded-full overflow-hidden">
                      <div className="h-full bg-blue-500" style={{ width: `${(branch.used / branch.tokensMonth) * 100}%` }} />
                    </div>
                  </div>
                </div>
                    ...
                    {DOCTOR_DATA.map((doc, i) => (
                      <div key={i} className="flex items-center gap-3 p-3">
                        <div className="size-10 rounded-md bg-zinc-200 shrink-0 overflow-hidden">
                          <Image src="/mini-placeholder.svg" alt={doc.name} width={40} height={40} className="object-cover h-full w-full" />
                        </div>
                        <div className="flex flex-col gap-1 flex-1">
                          <span className="text-sm font-medium text-black-500">{doc.name}</span>
                          <span className="text-xs text-black-300">{doc.tokensLeft}/{doc.maxTokens} tokens left</span>
                        </div>
                      </div>
                    ))}
                  </div>
