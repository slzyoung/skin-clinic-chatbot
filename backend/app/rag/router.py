...
async def process_document(file_path: str, knowledge_id: uuid.UUID, index: bool):
    try:
        async with async_session_maker() as session:
            ...
            # Dynamically fetch embedding model based on config/defaults
            ...
            pipeline = IngestionPipeline(vector_store=adapter, embeddings=embeddings)
            ...
            # Update status to PENDING (for manual review) and generate executive AI summary
...
async def ingest_document(
    ...
    try:
        knowledge_type = KnowledgeType(type.upper())
    except ValueError:
...
async def chat_endpoint(
    ...
    if request.knowledge_id:
        filter_metadata["knowledge_id"] = request.knowledge_id
